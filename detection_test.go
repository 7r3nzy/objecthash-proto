...
func TestIsUnsetWithBadArguments(t *testing.T) {
	t.Run("Should return errors for struct fields", func(t *testing.T) {
		...
		expectedError := "got an unexpected struct of type 'struct {}' for field {Name:s PkgPath:github.com/7r3nzy/objecthash-proto Type:struct {} Tag: Offset:0 Index:[0] Anonymous:false}"
	...
	t.Run("Should return errors for fields with unsupported types", func(t *testing.T) {
		...
		// Make sure that the returned error is meaningful.
		expectedError := "got an unexpected type 'chan interface {}' for field {Name:c PkgPath:github.com/7r3nzy/objecthash-proto Type:chan interface {} Tag: Offset:0 Index:[0] Anonymous:false}"
		if err.Error() != expectedError {
			t.Errorf("Expected the error returned by isUnset to be '%s'. Instead got '%s'.", expectedError, err)
		}
	})
